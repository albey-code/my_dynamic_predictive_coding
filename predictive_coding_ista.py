...
class DynPredNet(nn.Module):
    ...
    def inf(self, x, r_p, r2, record=False):
        batch_size = x.size(0)
        ...
        while not converged and i < self.max_iter:
            ...
            i += 1
            if record: 
                r_record = r.clone().detach()[None,:,:] if r_record is None else torch.cat((r_record, r.clone().detach()[None,:,:]), 0)
                r2_record = r2.clone().detach()[None,:,:] if r2_record is None else torch.cat((r2_record, r2.clone().detach()[None,:,:]), 0)
        self.converge_warning_(i, "r/r2 did not converge")
        if record:
            return r.clone().detach(), r2.clone().detach(), r2_loss, r_record, r2_record
        else:
